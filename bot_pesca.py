...
# sia "fishing" che il frutto "peach") - userei parole composte più specifiche
PAROLE_CHIAVE_PESCA = [
    "canna da pesca", "canne da pesca", "mulinello", "mulinelli",
    "amo da pesca", "ami da pesca", "esca da pesca", "esche da pesca",
    "lenza", "lenze", "galleggiante da pesca", "guadino",
    "abbigliamento da pesca", "wader", "waders", "kayak da pesca",
    "cassetta da pesca", "borsa da pesca", "carp fishing", "spinning pesca",
    "surfcasting", "traina", "bolentino", "feeder pesca", "carpfishing",
    "ecoscandaglio", "fishfinder", "filo da pesca", "monofilo pesca",
]
...
def e_prodotto_pesca(titolo):
    ...
    return any(parola in titolo_lower for parola in PAROLE_CHIAVE_PESCA)
...
def crea_immagine(prodotto):
    ...
    font_titolo = carica_font_locale(30)
    ...
    Y_PREZZO_VECCHIO = 890    # riga 1 del riquadro 3 (teal): prezzo barrato
    Y_SCONTO = 970            # riga 2 del riquadro 3 (teal): percentuale sconto

    titolo_txt = textwrap.fill(prodotto["titolo"][:55], width=24)
...
def cerca_asin_amazon(query):
    ...
    try:
        res = requests.get(url, headers=headers, timeout=12)
        if res.status_code != 200:
            print(f"[DEBUG RICERCA] '{query}' -> status_code={res.status_code}")
            return []
...
async def main():
    ...
    while True:
        for query in QUERY_RICERCA_PESCA:
            ...
            await asyncio.sleep(5)  # pausa tra una query e l'altra
