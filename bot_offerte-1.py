    ...
    try:
        ...
        for tag, attrs in candidati_prezzo:
            ...
            for p_elem in trovati_candidati:
                # DUMP della struttura reale, per capire come Amazon marca i prezzi
                print(f"[DUMP] {asin} -> elemento: {str(p_elem)[:300]}")
                print(f"[DUMP] {asin} -> genitore: {str(p_elem.parent)[:400]}")
                ...
                if not off_elem:
                    print(f"[DEBUG SCRAPER] {asin} -> scartato: nessun a-offscreen dentro {attrs}")
                    continue
                testo_prezzo = off_elem.get_text().strip()
                if not sembra_prezzo_valido(testo_prezzo):
